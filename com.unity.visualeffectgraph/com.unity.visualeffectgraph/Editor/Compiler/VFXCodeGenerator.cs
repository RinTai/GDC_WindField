...
",
        ...
        internal static Dictionary<VFXExpression, string> BuildExpressionToName(VFXContext context, VFXContextCompiledData taskData)
        {
        ...
        internal static void BuildContextBlocks(VFXContext context, VFXContextCompiledData contextData, Dictionary<VFXExpression, string> expressionToName,
            out VFXShaderWriter blockFunctionContent,
            out VFXShaderWriter blockCallFunctionContent,
            out VFXShaderWriter blockIncludesContent,
            out VFXShaderWriter blockDefinesContent)
        {
            //< Block processor
            blockFunctionContent = new VFXShaderWriter();
            blockCallFunctionContent = new VFXShaderWriter();
            
            blockIncludesContent = new VFXShaderWriter();
            blockDefinesContent = new VFXShaderWriter();
            ...
            foreach (var current in context.activeFlattenedChildrenWithImplicit)
            {
                ...
                {
                    blockFunctionContent.Write(hlslCodeHolder.customCode);
                    foreach (var includePath in hlslCodeHolder.includes)
                    {
                        if (includesProcessed.Add(includePath))
                        {
                            blockIncludesContent.WriteLine($"#include \"{includePath}\"");
                        }
                    }
                }
                ...
                foreach (var define in current.defines)
                {
                    ...
                    {
                        blockDefinesContent.WriteLineFormat("#define {0}{1}", define, define.Contains(' ') ? "" : " 1");
                    }
                }
                
                BuildBlock(contextData, blockFunctionContent, blockCallFunctionContent, blockDeclared, expressionToName, current, ref cpt);
            }

            // Custom HLSL Operators
            var customCodeProcessed = new HashSet<string>();
            foreach (var hlslCodeHolder in contextData.hlslCodeHolders)
            {
                var customCode = hlslCodeHolder.customCode;
                if (customCodeProcessed.Add(customCode))
                {
                    blockFunctionContent.Write(customCode);
                }
            ...
            blockIncludesContent = blockIncludes.builder.ToString();
            blockDefinesContent = blockDefines.builder.ToString();*/
        }
            ...
            VFXContextCompiledData contextData, HashSet<string> dependencies, bool enableShaderDebugSymbols)
        {
            ...
            var expressionToName = BuildExpressionToName(context, contextData);
            BuildContextBlocks(context, contextData, expressionToName, 
