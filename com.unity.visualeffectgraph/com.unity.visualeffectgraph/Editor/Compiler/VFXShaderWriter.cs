...
namespace UnityEditor.VFX
{
    ...
    class VFXShaderWriter
    {
        ...
        public void WriteBuffer(VFXUniformMapper mapper, ReadOnlyDictionary<VFXExpression, BufferUsage> usageGraphicsBuffer)
        {
            foreach (var buffer in mapper.buffers)
            {
                ...
                if (buffer.valueType == VFXValueType.Buffer && usageGraphicsBuffer.TryGetValue(buffer, out var type))
                {
                    ...
                    var structureName = GetBufferDeclaration(type);
