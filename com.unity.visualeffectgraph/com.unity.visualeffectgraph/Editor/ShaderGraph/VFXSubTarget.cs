...
namespace UnityEditor.VFX
{
    static class VFXSubTarget
    {
            ...
            out AdditionalCommandDescriptor additionalFragInputs)
        {
            ...
            // Graph Blocks
            var expressionToName = VFXCodeGenerator.BuildExpressionToName(context, contextData);
            VFXCodeGenerator.BuildContextBlocks(context, contextData, expressionToName,
                out var blockFunction, 
                out var blockCallFunction,
                out var blockIncludesContent,
                out var blockDefinesContent);

            perBlockDefines = new AdditionalCommandDescriptor("VFXPerBlockDefines", blockIncludesContent.builder.ToString());
            perBlockIncludes = new AdditionalCommandDescriptor("VFXPerBlockIncludes", blockDefinesContent.builder.ToString());
            ...
            expressionToName = context.GetData().GetAttributes().ToDictionary(o => new VFXAttributeExpression(o.attrib) as VFXExpression, o => (new VFXAttributeExpression(o.attrib)).GetCodeString(null));
