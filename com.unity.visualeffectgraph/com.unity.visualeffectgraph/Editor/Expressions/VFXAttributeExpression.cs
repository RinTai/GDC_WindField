...
namespace UnityEditor.VFX
{
    ...
    partial struct VFXAttribute
    {
        ...
        internal static readonly VFXAttribute StripAlive = new VFXAttribute("stripAlive", VFXValue.Constant(true)); // Internal attribute used to keep track of the state of the attached strip (TODO: Use a number to handle more tha 1 strip)
                ...
                [Tooltip("Outputs true at particle collision")]
        public static readonly VFXAttribute HasCollisionEvent =
            new VFXAttribute("hasCollisionEvent", VFXValue.Constant(false));// {category = kCollisionCategory };;

        [Tooltip("Outputs the collider normal at collision point at this frame. (0,0,0) if no collision")]
        public static readonly VFXAttribute CollisionEventNormal = new VFXAttribute("collisionEventNormal",
            VFXValue.Constant(Vector3.zero), VFXVariadic.False, SpaceableType.Direction);// {category = kCollisionCategory };

        [Tooltip("Outputs the collision point at this frame. (0,0,0) if no collision")]
        public static readonly VFXAttribute CollisionEventPosition = new VFXAttribute("collisionEventPosition",
            VFXValue.Constant(Vector3.zero), VFXVariadic.False, SpaceableType.Position);// {category = kCollisionCategory };

        [Tooltip("Outputs the number of total collisions detected by the particle since its birth")]
        public static readonly VFXAttribute CollisionEventCount =  new VFXAttribute("collisionEventCount", VFXValueType.Uint32);// {category = kCollisionCategory };
        ...
        [Tooltip("The velocity at the beginning of the context, before any force integration")]
        public static readonly VFXAttribute OldVelocity = new VFXAttribute("oldVelocity", VFXValueType.Float3);// {category = kAdvancedSimulationCategory };
        // Internal as we don't want it to appear in the graph
        [Tooltip("The particle’s Euler rotation on each axis. Expressed as Angle in degree. For Camera-facing billboard particles, the Z axis is most likely the desired axis of rotation.")]
        internal static readonly VFXAttribute angle = new VFXAttribute("angle", VFXValueType.Float3, VFXVariadic.True);// { category = kAdvancedSimulationCategory };

        [Tooltip("The angular rotation of the particle, in degrees per second. By default, the Update Context is responsible to calculate the Rotation by integrating the angularVelocity each frame.")]
        internal static readonly VFXAttribute angularVelocity = new VFXAttribute("angularVelocity", VFXValueType.Float3, VFXVariadic.True);// { category = kAdvancedSimulationCategory };

        [Tooltip("The point around which the particle rotates, moves and is scaled. By default, this is the center of the particle. The value is computed in a one unit box size. By default, it is (0,0,0), the center of the box. You can change its value to adjust the center of the box. Every face is located at -0.5 or 0.5 in each axis.")]
        internal static readonly VFXAttribute pivot = new VFXAttribute("pivot", VFXValueType.Float3, VFXVariadic.True);// { category = kRenderingCategory };

        [Tooltip("The non-uniform scale of the particle that act as a multiplier to its size.")]
        internal static readonly VFXAttribute scale = new VFXAttribute("scale",
            VFXValue.Constant(new Vector3((float)ScaleX.value.GetContent(), (float)ScaleY.value.GetContent(), (float)VFXAttribute.ScaleZ.value.GetContent())),
            VFXVariadic.True);// { category = kRenderingCategory };
